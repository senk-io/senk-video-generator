...
5. 保持推荐的“分阶段驻留”策略，检查 MPS 上限、启动内存、运行中停止阈值和最大新增换页预算。
6. 勾选高内存风险确认，点击“运行预检”。
...
| 档位 | 参数 | 用途 |
| --- | --- | --- |
| 内存探针 | `256×144`、`9` 帧、`1` 步 | 第一次验证内存边界，默认选项 |
| 低内存生成 | `416×240`、`9` 帧、`4` 步 | 探针闭合后的短视频试运行 |
| 既有兼容基线 | `416×240`、`17` 帧、`4` 步 | 只用于与既有高内存证据比较 |

### 执行策略

“分阶段驻留”使用 Diffusers 的模型级中央处理器卸载钩子，按文本编码器、Transformer、VAE 顺序把当前需要的模块移入 MPS；推理后主动释放钩子、删除管线引用、执行垃圾回收并清空 MPS 缓存。“全量驻留基线”仍保留为对比路径，但会把完整管线移入 MPS，不是默认选择。

MPS 上限用于快速失败和限制进程分配，不等于整个系统内存或换页空间的硬隔离。因此它必须与启动前可用内存、运行中停止阈值和最大新增换页护栏共同使用。
...
控制台测试使用临时假执行器，不加载模型、不产生真实视频。测试覆盖固定档位、MPS 上限、两种驻留策略、主动释放证据、请求范围、不可变登记、事件摘要链、二次确认、停止竞争、动态预检、会话令牌、路径穿越和本机绑定限制。
