...
4. 调整分辨率、帧数、推理步数、引导强度、帧率、种子和最长执行时间。
5. 检查启动内存、运行中停止阈值和最大新增换页预算。
6. 勾选高内存风险确认，点击“运行预检”。
...
控制台测试使用临时假执行器，不导入 PyTorch、不加载模型、不产生真实视频。测试覆盖请求范围、不可变登记、事件摘要链、二次确认、停止竞争、动态预检、会话令牌、路径穿越和本机绑定限制。
