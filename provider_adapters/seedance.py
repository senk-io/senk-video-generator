...
SEEDANCE_API_BASE = "https://ark.ap-southeast.bytepluses.com/api/v3"
SEEDANCE_API_HOST = "ark.ap-southeast.bytepluses.com"
SEEDANCE_API_KEY_ENV = "ARK_API_KEY"
SEEDANCE_MODEL_ID = "dreamina-seedance-2-0-260128"
SEEDANCE_PROVIDER_KEY = "seedance"
SEEDANCE_PROVIDER_IDENTITY = "BytePlus"
SEEDANCE_API_VERSION = "v3"
EXECUTION_ID_PATTERN = re.compile(r"[A-Z0-9][A-Z0-9._-]{2,127}")
TASK_ID_PATTERN = re.compile(r"cgt-[A-Za-z0-9_-]{1,128}")
...
def validate_trial_contract(value: Any) -> dict[str, Any]:
    ...
    if value.get("provider_key") != SEEDANCE_PROVIDER_KEY:
        raise ValueError("试验合同提供者必须是 seedance")
    provider = value.get("provider")
    if not isinstance(provider, dict):
        raise ValueError("试验合同缺少提供者配置")
    if provider.get("provider_identity") != SEEDANCE_PROVIDER_IDENTITY:
        raise ValueError("提供者身份必须固定为 BytePlus")
    if provider.get("model_id") != SEEDANCE_MODEL_ID:
        raise ValueError("模型标识必须固定为 dreamina-seedance-2-0-260128")
    if provider.get("api_version") != SEEDANCE_API_VERSION:
        raise ValueError("接口版本必须固定为 v3")
    if provider.get("execution_backend") != "remote_api":
        raise ValueError("Seedance 试验必须使用 remote_api 后端")
    ...
    if resolution not in {"480p", "720p", "1080p"}:
        raise ValueError("分辨率只允许 480p、720p 或 1080p")
    ...
    if generation.get("generate_audio") is not True:
        raise ValueError("第一片试验必须请求原生音频")
    if generation.get("watermark") is not False:
        raise ValueError("第一片试验不得请求水印")
...
def public_task_observation(response: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(response, dict):
        raise AdapterError("INVALID_QUERY_RESPONSE", "查询响应必须是对象")
    ...
    return {
        ...
        "generate_audio": response.get("generate_audio"),
        "framespersecond": response.get("framespersecond"),
        "usage": response.get("usage"),
        "error": response.get("error"),
        "output_url_present": isinstance(content.get("video_url"), str) and bool(content["video_url"]),
    }
...
class UrllibSeedanceTransport:
    ...
    def __init__(
        self,
        api_key: str,
        api_base: str = SEEDANCE_API_BASE,
        request_timeout_seconds: int = 60,
    ) -> None:
        if not api_key.strip():
            raise ValueError("Seedance 接口密钥不能为空")
        parsed = urlparse(api_base)
        if (
            parsed.scheme != "https"
            or parsed.netloc != SEEDANCE_API_HOST
            or parsed.path.rstrip("/") != "/api/v3"
        ):
            raise ValueError("Seedance 接口来源必须固定为 https://ark.ap-southeast.bytepluses.com/api/v3")
    ...
    def _json_request(
        ...
        request = Request(
            ...
            headers={
                ...
                "User-Agent": "senk-video-generator/seedance-adapter-v1",
            },
        )
        try:
            ...
            try:
                ...
                if isinstance(value, dict):
                    details["provider_error"] = value.get("error") or value
            except (UnicodeDecodeError, json.JSONDecodeError):
                details["response_body_available"] = bool(raw)
            raise AdapterError("REMOTE_HTTP_ERROR", "Seedance 接口返回错误状态", details) from exc
        except (TimeoutError, URLError) as exc:
            raise AdapterError("REMOTE_NETWORK_ERROR", "Seedance 接口网络请求失败", {"type": type(exc).__name__}) from exc
        try:
            value = json.loads(raw)
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise AdapterError("INVALID_JSON_RESPONSE", "Seedance 接口返回无效 JSON") from exc
        if not isinstance(value, dict):
            raise AdapterError("INVALID_JSON_RESPONSE", "Seedance 接口响应必须是对象")
        return value
    ...
    def query(self, task_id: str) -> dict[str, Any]:
        ...
        return self._json_request("GET", f"/contents/generations/tasks/{quote(task_id, safe='')}")

    def cancel(self, task_id: str) -> dict[str, Any]:
        if not TASK_ID_PATTERN.fullmatch(task_id):
            raise AdapterError("INVALID_TASK_ID", "提供者任务标识格式无效")
        return self._json_request("DELETE", f"/contents/generations/tasks/{quote(task_id, safe='')}")

    def download(self, url: str, destination: Path, max_bytes: int) -> dict[str, Any]:
        parsed = urlparse(url)
        if parsed.scheme != "https" or not parsed.netloc:
            raise AdapterError("INVALID_OUTPUT_URL", "输出下载地址必须是 HTTPS 地址")
        request = Request(url, method="GET", headers={"User-Agent": "senk-video-generator/seedance-adapter-v1"})
...
def run_trial(
    ...
    *,
    ...
    request_record = {
        ...
        "provider_key": SEEDANCE_PROVIDER_KEY,
        "provider_identity": SEEDANCE_PROVIDER_IDENTITY,
        "model_id": SEEDANCE_MODEL_ID,
        "api_version": SEEDANCE_API_VERSION,
    ...
    try:
        ...
        if not TASK_ID_PATTERN.fullmatch(task_id):
            raise AdapterError("INVALID_CREATE_RESPONSE", "创建响应缺少有效任务标识")
        write_json(
            evidence_dir / "provider_submission.json",
            {"task_id": task_id, "model": SEEDANCE_MODEL_ID, "submitted_at": utc_now()},
        )
        ...
        while monotonic() - started <= normalized["timeout_seconds"]:
            ...
            if observation["model"] != SEEDANCE_MODEL_ID:
            ...
            if observation["status"] in TERMINAL_STATUSES:
                ...
                break
            if observation["status"] not in IN_PROGRESS_STATUSES:
        ...
        content = final_response.get("content") if isinstance(final_response.get("content"), dict) else {}
        output_url = content.get("video_url")
        ...
        if output_metadata is not None:
            ...
            if not output_metadata.get("audio_stream_present"):
                raise AdapterError("OUTPUT_AUDIO_MISSING", "Seedance 输出缺少预期的原生音频流")
        ...
        if task_id and (final_observation is None or final_observation.get("status") in IN_PROGRESS_STATUSES):
