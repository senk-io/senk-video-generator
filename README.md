...
控制台默认使用固定“内存探针”档位、文本编码器提前释放、分阶段 MPS 驻留和 `75%` 建议工作集上限。预检同时要求至少 `16 GiB` 可用内存和不超过 `4 GiB` 现有换页；高换页恢复期禁止启动。预检和登记不会加载模型，只有完整执行标识二次确认后才会启动真实作业。新组件驻留策略的实际峰值与耗时仍需通过单独授权的真实探针建立，不能从旧记录推断。
