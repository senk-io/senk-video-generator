...
控制台默认使用固定“内存探针”档位、文本编码器无梯度叶级顺序卸载、阶段完成后提前释放和 `75%` MPS 建议工作集上限。预检同时要求至少 `16 GiB` 可用内存和不超过 `4 GiB` 现有换页；高换页恢复期禁止启动。预检和登记不会加载模型，只有完整执行标识二次确认后才会启动真实作业。该组合已在当前 Mac 上以 `256×144`、`9` 帧、`1` 步完成一次受控生成与证据闭包；它只证明最低成本路径可运行，不表示画面质量或更高档位已经通过。

## 30 秒样片

当前首个长内容阶段固定为六个 5 秒镜头的 30 秒样片。控制台顶部可以逐镜头载入固定提示词、登记和运行候选；完成输出仍需本地操作者显式选择，六镜头选择齐全后才能组装结构样片。项目合同、操作路径、状态和质量边界见 [`docs/30-second-pilot.md`](docs/30-second-pilot.md)。
