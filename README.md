...
理解能力。当前 `v10` 套件对雨中哭泣特写、室内微笑中景和自行车横穿街道全景执行
`63/63` 次本地调用；九轮均能解析，但 `0/9` 形成可比较草案。候选中文释义把保留
观察从 `v9` 的 `153` 项降到 `120` 项，仍未消除场景连续性、枚举选择和主体运动被
误判为相机运动的问题。因此该 `0.6B` 路径当前只能提供诊断观察，不能自动批准镜头。
...
| 自动回归 | `144` 个单元测试和 `1` 个迁移测试通过，测试本身不下载或运行模型 |
