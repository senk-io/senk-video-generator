...
控制台默认使用固定“内存探针”档位、文本编码器叶级顺序卸载、阶段完成后提前释放和 `75%` MPS 建议工作集上限。预检同时要求至少 `16 GiB` 可用内存和不超过 `4 GiB` 现有换页；高换页恢复期禁止启动。预检和登记不会加载模型，只有完整执行标识二次确认后才会启动真实作业。叶级顺序卸载的实际峰值与耗时仍需通过单独授权的真实探针建立，不能从旧记录推断。
