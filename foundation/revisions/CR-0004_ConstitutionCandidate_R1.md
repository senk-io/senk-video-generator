...
> 本文件是 `CR-0004` 的单一宪制候选 R1，不是冻结制度。它有界修复首次候选一致性审查的 B1 至 B4，不覆盖原候选或任何历史来源。它不创建制度注册表、冻结账本、冻结标识、冻结决定、启动模式或运行时权威，也不能授权任何运行时写入。
...
精确内容同一关系和完整边界支持 `REGISTERED`；合格、适用、完整的未登记证明支持 `NOT_REGISTERED`；不兼容权威关系或同键不同解析支持 `CONFLICTED`；缺失、边界不完整、证明资格不可用或读取失败支持 `INDETERMINATE`。只有已登记 `REGISTERED` 可以进入生命周期竞争集合。

```text
Candidate Lifecycle Relation Registration Resolution Record
  -> Lifecycle Relation Registration Resolution Registration Attempt
  -> Registered Lifecycle Relation Registration Resolution Record

Candidate Lifecycle Relation Registration Resolution Payload Digest
= Registered Lifecycle Relation Registration Resolution Payload Digest
```
...
该键不得包含 `Lifecycle Decision Fact ID`、记录标识、记录时间、插入顺序或写入者。目标分量必须按域规范化：

```text
SOURCE_VERSION_APPLICABILITY -> NOT_APPLICABLE
DEPRECATION_SIGNAL -> NOT_APPLICABLE
SUCCESSOR_SELECTION -> ALL_SUCCESSOR_TARGETS through IR-CC-022 parent key
```

精确继任目标只保留在成员载荷中，不能进入竞争集合键。
...
```text
Candidate Successor Selection Aggregate Resolution Payload Digest
= Registered Successor Selection Aggregate Resolution Payload Digest
```

精确单一非替代目标或兼容收敛目标与完整边界产生 `UNIQUE_SUCCESSOR`；合格适用完整的无关系证明产生 `NO_SUCCESSOR`；不同非替代目标、不完整或循环替代、同键不兼容聚合产生 `CONFLICTED`；来源、完整性或证明资格未知产生 `INDETERMINATE`。
...
候选和登记记录必须共同绑定复合键、域向量、三个域解析、全部来源边界及完整性、结果、继任目标或 `NOT_APPLICABLE`、弃用注释、规则版本、权威、双时间、证据和内容同一摘要。

```text
Candidate Composite Lifecycle Applicability Resolution Payload Digest
= Registered Composite Lifecycle Applicability Resolution Payload Digest
```
...
```text
any required domain = CONFLICTED
or successor aggregate = CONFLICTED
or incompatible registered composite results for same key
-> CONFLICTED

no CONFLICTED
+ any required domain = INDETERMINATE
...
同键同载荷幂等；同键不同载荷必须为 `CONFLICTED`，不得最后写入获胜。
...
`NATIVE + NATIVE_FREEZE` 必须精确绑定：

```text
Exact Institution Content Artifact and Digest
Registered ALLOCATED NATIVE_NEW Freeze ID Resolution and Record
Registered Institution Proposal
Approved Independent Review Decision
...
`BOOTSTRAP_RECOGNIZED + PRESERVED_PRE_REGISTRY_FREEZE` 必须同时绑定完整旧冻结链和启动登记链：

```text
Exact Artifact and Bootstrap-observed Digest
Exact Complete Pre-registry Freeze Evidence Package
Verified Pre-registry Freeze ID, Authority and Decision
Registered ALLOCATED BOOTSTRAP_RESERVED_EXISTING Resolution and Record
Registered COMMITTED Pre-registry Commit Resolution
Successful Pre-registry Institution Commit Reference
Registered Manifest and COMMITTED External Anchor Resolution
Bootstrap Recognition Decision selecting PRESERVED mode
Registered ACTIVE_CLOSED Bootstrap Mode Resolution and all sources
Exact Bootstrap Registry and Recognition Ledger Entries
Registered Composite Lifecycle Applicability
COMPLETE Boundary Records for every source
```

启动识别决定只能确认使用完整旧链，不能替换旧链中的冻结标识、决定、权威、证据、提交或原有效时间。

`BOOTSTRAP_RECOGNIZED + PROSPECTIVE_BOOTSTRAP_RECOGNITION` 必须绑定：

```text
Exact Artifact and Bootstrap-observed Digest
Bootstrap Evidence Package
Independent Bootstrap Review Decision
Registered ALLOCATED NATIVE_NEW Bootstrap Freeze ID Resolution and Record
Applicable Bootstrap Freeze Recognition Authority
Bootstrap Recognition Decision selecting PROSPECTIVE mode
Registered Manifest and COMMITTED External Anchor Resolution
Registered ACTIVE_CLOSED Bootstrap Mode Resolution and all sources
Exact Bootstrap Registry and Recognition Ledger Entries
Validity Start no earlier than registered Bootstrap COMMITTED At
Registered Composite Lifecycle Applicability
COMPLETE Boundary Records for every source
```

前瞻模式中的旧声明冻结标识和时间只能作为历史字段，不得成为新冻结引用的 `Freeze ID` 或有效起点。
...
`HISTORICAL_AS_KNOWN` 只消费 `Recorded At <= Known At` 且在 `Valid At` 适用的记录；`CURRENT_RESTATED` 可以消费当前完整边界内的后续更正、生命周期和冲突记录，但必须形成独立投影，不能覆盖历史当时认识。
...
任一字段变化必须形成新的候选和已登记解析身份，不得覆盖既有解析。
...
```text
Candidate Freeze Reference Resolution Payload Digest
= Registered Freeze Reference Resolution Payload Digest
```
...
qualified applicable complete proof of no registered manifest for exact key
+ manifest carrier boundary COMPLETE
-> NOT_REGISTERED

multiple manifest contents for same key
or different first-entry sets, window cores, authority bundles or canonicalization contracts
or incompatible registered manifest resolutions for same resolution key
-> CONFLICTED
...
External Bootstrap Anchor Commitment Candidate
  ...
  -> Candidate External Anchor Commitment Resolution Record
  -> External Anchor Commitment Resolution Registration Attempt
  -> Registered External Anchor Commitment Resolution Record
```

同锚键同载荷幂等；同键不同载荷必须保留冲突。精确内容同一锚记录与完整外部边界支持 `COMMITTED`；合格、适用、完整的无写入证明支持 `ABORTED`；不同清单摘要、不兼容权威锚或同解析键不同结果支持 `CONFLICTED`；缺失、不完整或读取失败支持 `INDETERMINATE`。候选与登记载荷必须内容同一。
...
窗口只绑定该核心键，核心键不得包含最终窗口记录摘要、内部提交结果边界或任何证据、封印、终局和模式载体。

清单及外部锚登记后只能构造 `Candidate Bootstrap Window Definition Record`。内部启动尝试必须绑定该精确候选；已登记窗口只能作为内部保护提交的一部分，与候选内容同一地成立。
...
该键不得包含封印记录标识、封印摘要、证据集合摘要、封印时间或封印账本位置。
...
Candidate Bootstrap Commit Evidence Boundary Seal Payload Digest
= Registered Bootstrap Commit Evidence Boundary Seal Payload Digest
```
...
R1 在原候选来源谱系之外增加以下有界修复谱系：

| 首次审查阻断 | R1 修复位置 | 规范来源 |
|---|---|---|
| B1 生命周期、更正与复合身份 | `IR-CC-019..024` | `IR-R1-19..27`、`IR-R2-12..23`、`IR-R3-11..20` |
| B2 冻结引用解析与三模式 | `IR-CC-026..029` | `IR-R1-46..55`、`IR-R3-08..10` |
| B3 启动控制对象身份 | `IR-CC-032..039` | `IR-R1-34..40`、`IR-R2-24..31`、`IR-R3-21..27` |
| B4 窗口保护提交因果 | `IR-CC-034`、`IR-CC-053` | `IR-R1-37..40`、`IR-R2-27..34` |
...
Open First Candidate Review Blockers: 0
Independent Consistency Review: REQUIRED
Institution Freeze Eligibility: NOT_ASSESSED
First Candidate Review Blocker B1: CLOSED_AS_R1_CANDIDATE
First Candidate Review Blocker B2: CLOSED_AS_R1_CANDIDATE
First Candidate Review Blocker B3: CLOSED_AS_R1_CANDIDATE
First Candidate Review Blocker B4: CLOSED_AS_R1_CANDIDATE
```

## 当前决定

```text
CR-0004 Constitution Candidate R1: CREATED
Status: CONSISTENCY_REVIEW_REQUIRED
Authority: NONE
Executable: NO
Repair Scope: B1 + B2 + B3 + B4 only
Bounded Repair: COMPLETED_AS_CANDIDATE
Independent R1 Consistency Review: REQUIRED
WS-01 Exit: BLOCKED_PENDING_CONSISTENCY_REVIEW
Institution Freeze: NOT_CREATED
Runtime Authority: NOT_CREATED
```

下一阶段只能执行 `CR-0004-CONSTITUTION-CANDIDATE-R1` 独立一致性复审。复审通过前不得退出 `WS-01`；复审通过也不等于创建冻结、注册表或运行时权威。
