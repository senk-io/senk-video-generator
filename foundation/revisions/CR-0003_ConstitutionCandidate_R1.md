...
```text
Proposal ID: CR-0003-CONSTITUTION-CANDIDATE-R1
Title: Commit Model
Status: CONSTITUTION_CANDIDATE
Authority: NONE
Executable: NO
Revision Type: BOUNDED_CONSOLIDATION_REPAIR
Revises: CR-0003-CONSTITUTION-CANDIDATE
Consolidates: CR-0003-R4 + CR-0003-R5 + CR-0003-R6 + CR-0003-R7
Review Basis: CR-0003-CONSTITUTION-CANDIDATE-CONSISTENCY-REVIEW
Proposer: Codex under user-authorized candidate repair
...
> 本文件是单一宪法候选稿第一修订版，不是冻结制度。它只修复第一次合并的完整性阻断，不能授权运行时提交、改变正式事实、覆盖冻结原则或使任何依赖自动冻结。原候选、历史草案和审查记录继续作为不可覆盖的治理历史保存。
...
投影必须绑定规范类型、`Projection View Mode`、作用域、时间坐标、规则版本、来源集合、依赖闭包、谱系摘要、冲突引用和生成时间。投影缓存可删除和重建。
...
`Projection Change Audit Registrar` 必须执行内容同一性和授权检查，不能修改候选审计、候选投影或视图模式。候选投影、变化审计和发布外壳的 `Projection View Mode` 必须完全一致。审计追加失败时，新投影发布必须阻断；候选仍可本地重建，旧发布快照保持历史。
...
建议动作：对本 R1 单一候选稿重新执行对象图、类型、权威、因果、时间、认识上限和术语一致性审查。发现遗漏或语义冲突时建立新的候选修订，不修改历史候选与草案，也不得直接冻结。
