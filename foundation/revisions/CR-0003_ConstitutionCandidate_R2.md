...
```text
Proposal ID: CR-0003-CONSTITUTION-CANDIDATE-R2
Title: Commit Model
Status: CONSTITUTION_CANDIDATE
Authority: NONE
Executable: NO
Revision Type: BOUNDED_CONSOLIDATION_REPAIR
Revises: CR-0003-CONSTITUTION-CANDIDATE-R1
Consolidates: CR-0003-R4 + CR-0003-R5 + CR-0003-R6 + CR-0003-R7
Review Basis: CR-0003-CONSTITUTION-CANDIDATE-R1-CONSISTENCY-REVIEW
Proposer: Codex under user-authorized bounded candidate repair
...
> 本文件是单一宪法候选稿第二修订版，不是冻结制度。它只修复 R1 复审确认的两个局部阻断和残留术语，不能授权运行时提交、改变正式事实、覆盖冻结原则或使任何依赖自动冻结。原候选、R1、历史草案和审查记录继续作为不可覆盖的治理历史保存。
...
| `Source Applicability Provider` | 外部接口角色 | 提供来源所属制度的版本化适用性 | 来源模型权威边界 |
| `Source Applicability Input` | 接口值对象 | 引用上游来源的适用性结论 | 由来源模型提供 |
| `Applicability Change Record` | 不可变接口记录 | 追加通知来源适用性变化 | 来源权威边界 |
| `Qualification Projection Builder` | 执行角色 | 构建指定键和视图模式的资格投影 | 无正式事实所有权 |
| `Proof Qualification Projection` | 派生读面 | 表达指定坐标和视图模式的证明资格 | 可删除并重建 |
...
| `Dependency Closure Reference` | 接口值对象 | 绑定闭包、完整性和时间坐标 | 依附于投影输入 |
| `Temporal Query Resolver` | 执行角色 | 绑定有效时点、认识边界和视图模式 | 无来源修改权威 |
| `Projection Builder` | 执行角色 | 构建有认识上限的候选投影 | 无正式事实所有权 |
| `Projection Publisher` | 发布角色 | 发布已完成审计登记的派生读面 | 无正式事实所有权 |
| `Resolution Projection` | 派生读面 | 表达指定坐标和视图模式的解析认识 | 可删除并重建 |
| `Candidate Projection Change Audit Record` | 不可变候选记录 | 保存尚未登记的投影变化审计 | 未登记投影审计账本 |
| `Projection Change Audit Registrar` | 登记角色 | 登记内容相同的投影变化审计 | 无投影修改权威 |
| `Registered Projection Change Audit Record` | 追加式派生审计记录 | 保存获登记的投影变化审计 | 投影审计账本 |
...
| `Projection View Mode` | 枚举值 | 区分历史认识和当前重述 | 依附于投影身份 |
| `Qualification Projection Key` | 复合标识值 | 唯一标识资格投影 | 资格投影规则 |
| `Qualification Scope Mode` | 枚举值 | 选择精确契约或兼容域快照 | 资格投影键 |
| `Qualification Compatibility Domain Snapshot` | 不可变制度快照 | 固定可比较契约版本集合 | 资格治理制度 |
| `Qualification Semantic Compatibility Record` | 版本化制度记录 | 声明资格结果能否比较 | 资格治理制度 |
| `Compatibility Semantic Domain` | 枚举值 | 限定兼容关系的解释语义域 | 兼容契约 |
| `Epistemic Strength` | 偏序值 | 表达解析结果的认识强度 | 解析与投影规则 |
| `Qualification Epistemic Strength` | 偏序值 | 表达资格结果的认识强度 | 资格投影规则 |
| `Canonical Target State Snapshot` | 不可变值或引用 | 表达已解析目标状态载荷 | 目标状态解析记录 |
...
| `Institutional Source Exclusion Basis` | 版本化制度契约 | 定义来源排除的合法条件 | 来源治理制度 |
| `Institution Freeze Reference` | 制度引用值 | 证明制度版本合法冻结 | 制度注册表 |
| `Forward Interpreter` | 执行角色 | 按冻结契约执行非放大解释 | 无重新解析权威 |
| `Institution Freeze Reference Resolver` | 执行角色 | 验证冻结引用 | 无制度创建权威 |
...
### CM-C-17A 来源适用性采用最小消费接口

提交模型只消费来源所属制度提供的不可变接口，不定义全局失效传播算法：
...
接口记录必须不可变、版本化、可审计并属于来源模型权威边界。`Source Applicability Provider` 不能修改消费方历史资格、解析或投影。
...
更正只追加新的证据历史。原证据能否继续用于指定证明、资格或解析，必须由来源所属制度通过适用性解析决定。提交模型和投影器不得自行解释全局证据更正语义。

适用性变化只能触发派生缓存失效、依赖闭包重建和投影重算；不得修改历史证据、资格或解析，不得创建提交结果、目标状态或重试授权。
...
Underlying Qualification, Applicability, Closure and Evidence Record References
```

任一项未决时，提交解析只能保持 `INDETERMINATE`。投影摘要只能用于可重建校验，不能成为 `ABORTED` 的唯一来源依据。
...
Target State Resolution:
  RESOLVED(Canonical Target State Snapshot) | INDETERMINATE
```

读取可用不保证状态可解析；读取不可用不删除此前权威状态。解析记录只能保存直接制度原因，不能混入技术根因诊断。

### CM-C-27A 目标状态解析必须绑定规范状态载荷
...
Temporal Query Coordinate
Projection View Mode when projected
Source Record References
Evidence References
```

同一目标、状态主题、时间坐标和语义域内，规范摘要相同的 `RESOLVED` 表达同一终局含义；规范摘要不同的可比较 `RESOLVED` 来源形成目标状态投影冲突。

读取 `UNAVAILABLE` 只能形成读取原因；来源适用性未知或状态无法唯一重建只能形成解析 `INDETERMINATE` 原因。两类原因不得使用同一规范原因码互相替代。
...
每项资格、提交和目标状态投影、投影变化审计及发布外壳必须绑定唯一 `Projection View Mode`。`Current ... Projection` 只能作为 `CURRENT_RESTATEMENT_VIEW` 的显示别名，不能作为规范类型名称。
...
Target State:
  ...
  different comparable RESOLVED values are incomparable terminals
```
...
Comparable applicable RESOLVED(value_a) + RESOLVED(value_b)
+ Canonical Digest A != Canonical Digest B
  -> Target State Projection = CONFLICTED
```
...
每项兼容关系必须声明唯一 `Compatibility Semantic Domain`：
...
任何产生新状态值、改变规范含义或把不确定提升为 `RESOLVED` 的变化，必须重新执行目标状态解析并追加新的候选与登记历史。
...
目标状态终局投影必须保存规范快照、规范摘要、目标权威版本和来源状态解析记录引用；冲突投影必须保存全部可比较冲突快照、摘要和来源引用。

所有投影必须绑定规范类型、`Projection View Mode`、作用域、时间坐标、规则版本、来源集合、依赖闭包、谱系摘要、冲突引用和生成时间。投影缓存可删除和重建。
...
首次投影使用 `NOT_APPLICABLE` 作为前一摘要。候选或已登记审计都不是正式事实，也不能成为投影来源真值。
...
`Projection Change Audit Registrar` 必须执行内容同一性和授权检查，不能修改候选审计、候选投影或视图模式。候选投影、已登记审计和发布外壳的 `Projection View Mode` 与投影摘要必须完全一致。

候选审计即使未登记或登记失败也必须保留在未登记投影审计账本。没有内容相同的 `Registered Projection Change Audit Record` 时，新投影发布必须阻断；候选仍可本地重建，旧发布快照保持历史。

### CM-C-52 投影不产生正式事实或未来行动

```text
Resolution Projection or Proof Qualification Projection
!= Formal Fact
!= Decision Fact
...
| `Source Applicability Provider` | 提供来源制度拥有的版本化适用性接口 | 修改消费方历史或决定提交结果 |
...
| `Temporal Query Resolver` | 绑定有效时点、认识边界和视图模式 | 创建来源记录、扩大查询边界 |
| `Forward Interpreter` | 按冻结契约进行非放大解释 | 提高确定性、改变作用域 |
| `Projection Builder` | 使用完整输入构建候选投影 | 修改历史、创建正式事实 |
| `Projection Change Audit Registrar` | 登记内容相同的投影变化审计 | 修改投影、审计内容或视图模式 |
...
- 后来记录静默进入历史认识视图；
- 当前重述冒充历史认识；
- 投影、审计和发布外壳缺少或使用不同 `Projection View Mode`；
- 用 `Current ... Projection` 作为历史认识视图的规范类型；
- 用单一时间戳替代多注册表边界向量；
- 使用因果不闭合的边界产生终局投影；
...
- 忽略适用的相反资格或终局解析；
- 目标状态 `RESOLVED` 缺少规范状态载荷、目标版本或状态摘要；
- 不同规范状态摘要被当作同一目标状态终局；
- 把不兼容规则误报为事实冲突；
- 跨提交解析、证明资格和目标状态语义域复用兼容关系；
...
- 冻结引用无效时仍使用契约；
- 投影审计未登记却发布新投影；
- 未登记或登记失败的候选投影审计被删除或静默丢弃；
- 发布外壳引用候选审计而不是已登记审计；
- 投影变化审计登记者修改候选投影、审计内容或视图模式；
...
Qualification Projection -> determines proof usability at a declared view mode
Commit Resolution -> classifies one commit attribution
Target State Resolution -> classifies target state at a coordinate
Resolution Projection -> derives usable knowledge at a declared view mode
...
| `CR-0003-CONSTITUTION-CANDIDATE-CONSISTENCY-REVIEW` | 来源适用性接口、投影视图身份、目标状态认识模型和统一角色类型修复要求 |
| `CR-0003-CONSTITUTION-CANDIDATE-R1-CONSISTENCY-REVIEW` | 目标状态投影载荷、审计候选—登记链和投影术语清理要求 |
...
Candidate Bounded Repair R1: COMPLETED
Candidate Bounded Repair R2: COMPLETED
...
建议动作：对本 R2 单一候选稿执行最后一次对象图、类型、权威、因果、时间、认识上限和术语一致性复审。发现遗漏或语义冲突时建立新的候选修订，不修改历史候选与草案，也不得直接冻结。
