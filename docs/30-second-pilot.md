...
当前逐镜头进展：`SHOT-001` 已形成精确五秒折纸候选并通过技术连续性阈值，但仍等待按完整镜头标识进行人工选择。`SHOT-002` 的九帧提示词探针保留了折纸结构与水面语义，但主体首尾净向左约 `18.77` 像素。首次方向派生虽然改为向右，却因跨帧混合产生明显双轮廓；第二次纯空间派生已经消除重影，九个相邻步骤全部向右，首尾净向右约 `33.54` 像素。其最大相邻跳变为 `6.10` 像素，仍超过 `5.5` 像素门禁，且来源固有的面积波动未被修复。因此第二镜头形成了方向控制候选，但尚未取得四十一帧扩展许可。
