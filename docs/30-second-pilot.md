...
当前逐镜头进展：`SHOT-001` 已形成精确五秒折纸候选并通过技术连续性阈值，但仍等待按完整镜头标识进行人工选择。`SHOT-002` 的九帧提示词探针保留了折纸结构与水面语义，但主体首尾净向左约 `18.77` 像素。首次方向派生把首尾净位移改为向右约 `31.80` 像素，但跨帧混合造成明显双轮廓。现已单独冻结纯空间方向派生合同：重新绑定原始来源、保持向右轨迹、完全移除跨帧混合，并只允许一次无模型执行。在专用校验和九帧人工复核闭合前，第二镜头仍不得扩大到四十一帧。
