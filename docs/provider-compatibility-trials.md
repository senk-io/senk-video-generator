...
控制台会把这些值写入 `operator-job.v3` 作业请求。启动前必须同时满足至少 `16 GiB` 可用内存和不超过 `4 GiB` 现有换页。运行器先设置进程级 MPS 上限，再单独装载文本编码器形成提示词嵌入并释放，之后才装载 Transformer 与 VAE；推理后释放钩子、管线引用和 MPS 缓存。运行证据分别记录限制配置、组件驻留策略、文本编码器释放、MPS 峰值和最终释放，不能只凭页面选择断言策略已经生效。
...
文本编码器提前释放策略已经实现并通过无模型回归测试，但在该新策略下的实际峰值、耗时、换页增长和输出能力尚未观察。不得用旧的全量驻留成功记录或第一次模型级卸载失败记录替代新组件驻留策略的现实证据。
