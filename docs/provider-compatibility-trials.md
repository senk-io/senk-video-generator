...
控制台会把这些值写入 `operator-job.v2` 作业请求。运行器先设置进程级 MPS 上限，再启用 Diffusers 的模型卸载钩子；推理后释放钩子、管线引用和 MPS 缓存。运行证据分别记录限制配置、策略激活、峰值和释放后分配，不能只凭页面选择断言策略已经生效。
...
观测台的完整说明见 `observatory/README.md`。它不启动或控制试运行；低层命令行方式仍要求独立明确授权，并且不会替代控制台的风险确认体验。下面不带作业规格的命令读取历史固定合同，采用全量 MPS 驻留基线，不具备 `operator-job.v2` 的分阶段驻留和进程级比例限制；它只用于复查旧证据，不应作为第一次低内存试运行入口。
