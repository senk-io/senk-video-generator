#!/usr/bin/env python3
"""Preflight or run one explicitly billed MiniMax H3 remote trial."""
...
DEFAULT_CONTRACT = CONTRACT_ROOT / "minimax_h3_fictional_child_crying_closeup_v1.json"
...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    if not resolved.is_relative_to(CONTRACT_ROOT) or resolved.suffix != ".json":
        raise ValueError("H3 试验合同必须位于受控试验目录")
    try:
        value = json.loads(resolved.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError("无法读取 H3 试验合同") from exc
...
def validate_media_contract(metadata: dict[str, Any], contract: dict[str, Any]) -> None:
    ...
    if len(size) != 2 or min(size) != 768:
        raise AdapterError("OUTPUT_RESOLUTION_MISMATCH", "输出短边不是合同要求的 768 像素")
    ...
    if not metadata.get("audio_stream_present"):
        raise AdapterError("OUTPUT_AUDIO_MISSING", "H3 输出缺少原生音频流")
    if metadata.get("audio_sample_rate_hz") != 32000:
        raise AdapterError("OUTPUT_AUDIO_RATE_MISMATCH", "H3 输出音频采样率不是 32000 Hz")
    if metadata.get("audio_channels") != "stereo":
        raise AdapterError("OUTPUT_AUDIO_CHANNEL_MISMATCH", "H3 输出音频不是双声道")
...
def environment_record(execution_id: str, contract_path: Path) -> dict[str, Any]:
    return {
        ...
        "adapter_sha256": sha256_file(REPO_ROOT / "provider_adapters" / "minimax_h3.py"),
        "runner_sha256": sha256_file(Path(__file__)),
        "contract_sha256": sha256_file(contract_path),
        "api_origin": MINIMAX_H3_API_BASE,
        "credential_env": MINIMAX_H3_API_KEY_ENV,
        "credential_present": True,
        "credential_recorded": False,
        "local_open_weight_execution": "BLOCKED_BY_36GB_MEMORY_AND_UNVALIDATED_MPS_QUANTIZATION",
        "sensitive_machine_identifiers_recorded": False,
    }


def observe_preflight(contract: dict[str, Any], *, execute: bool) -> dict[str, Any]:
    key_present = bool(os.environ.get(MINIMAX_H3_API_KEY_ENV, "").strip())
    return {
...
def main() -> int:
    ...
    transport = UrllibMiniMaxH3Transport(os.environ[MINIMAX_H3_API_KEY_ENV])
    ...
    try:
        ...
        print(f"H3 试验失败：{exc}", file=sys.stderr)
        exit_code = 1
    finally:
