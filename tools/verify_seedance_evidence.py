#!/usr/bin/env python3
"""Independently verify a Seedance / BytePlus ModelArk remote-trial evidence package."""
...
PRIVATE_TEXT_PATTERNS = (
    ...
    re.compile(r"ARK_API_KEY\s*=\s*\S+"),
...
def verify(evidence_dir: Path) -> dict[str, Any]:
    ...
    if request.get("model_id") != "dreamina-seedance-2-0-260128" or summary.get("model_id") != "dreamina-seedance-2-0-260128":
        raise ValueError("模型标识不是 dreamina-seedance-2-0-260128")
    ...
    if payload.get("model") != "dreamina-seedance-2-0-260128":
        raise ValueError("请求载荷模型标识不一致")
    if payload.get("resolution") != "720p" or payload.get("duration") != 5 or payload.get("ratio") != "16:9":
        raise ValueError("请求载荷偏离固定生成合同")
    if payload.get("generate_audio") is not True or payload.get("watermark") is not False:
        raise ValueError("请求载荷音频或水印合同不一致")
    if "seed" in payload:
        raise ValueError("Seedance 2.0 请求不得携带 seed")
    for field in (
        "formal_fact_created",
    ...
    if summary.get("output_export_completed"):
        ...
        if "video_url" in final_task:
            raise ValueError("终态观察不得包含签名下载地址")
    ...
    return {
        ...
        "verification_result": "VERIFIED_SEEDANCE_OBSERVATION_PACKAGE",
    }
